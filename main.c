#include <stdio.h>
#include <stdlib.h>
#include <time.h>

// Global constants
#define X_LEN 134
#define MAX_NUM 10
#define NUMS 11
time_t t;

// Functions for filing arrays:
void load_arr(int arr[], int len);
void zero_arr(int arr[], int len);
void load_arr_from_file(char f_name[],int arr[], int len);


// Analytic functions:
void build_histogram(int hist[], const int in_arr[], int len);
int max(int pole[],int n);

// Functions for plotting graph
void plot_axis(char plain[][X_LEN], int Y_LEN);
void plot_graph(char plain[][X_LEN], int Y_LEN, int hist[]);
void print_graph_on_screen(char plain[][X_LEN], int Y_LEN);
void append_hist_to_file(char *f_name, int *hist, int len);

// Functions for generating pseudorandom numbers:
int get_rand();
void fill_arr_with_rand(int *arr, int len);

// Driver program:
int main()
{

    int all_elements[MAX_NUM];
    int histogram[NUMS];

    //load_arr(all_elements,MAX_NUM);
    //fill_arr_with_rand(all_elements, MAX_NUM);
    //load_arr_from_file("input.txt",all_elements,X_LEN);

    zero_arr(histogram, NUMS);

    //build_histogram(histogram, all_elements, MAX_NUM);
    load_arr_from_file("input.txt",histogram,NUMS);

    append_hist_to_file("output.txt", histogram, NUMS);

    int Y_LEN = max(histogram, NUMS);
    // printf("%d",Y_LEN);

    char graf[Y_LEN+1][X_LEN];

    plot_axis(graf,Y_LEN+1);

    plot_graph(graf,Y_LEN+1, histogram);

    printf("\n\n\n\n");
    print_graph_on_screen(graf,Y_LEN+1);




    return 0;
}



int max(int pole[], int n)
{
    int max = pole[0];
    for (int i = 1; i < n; ++i)
    {
        if (pole[i] > max)
          max  = pole[i];

    }
    return max;
}


void load_arr(int arr[], int len)
{
    for (int i = 0; i < len; ++i)
        scanf("%d", &arr[i]);
}

void load_arr_from_file(char f_name[],int arr[], int len)
{

    FILE *fp = fopen(f_name,"r");

    if(fp==NULL)
        perror("Fatal Error: ");
    else
    {
        for (int i = 0; i < len; ++i)
            fscanf(fp, "%d", &arr[i]);
        fclose(fp);
    }
}

void zero_arr(int arr[], int len)
{
    for (int i = 0; i < len; ++i)
        arr[i] = 0;
}

void build_histogram(int hist[], const int in_arr[], int len)
{
    for (int j = 0; j < len; ++j)
    {
        hist[in_arr[j]]++;
    }
}

void plot_axis(char plain[][X_LEN], int Y_LEN)
{

    for (int y = 0; y < Y_LEN ; ++y)
        for (int x = 0; x < X_LEN; ++x)
            plain[y][x]=' ';



    char j = '1';
    char j2 = '0';
    for (int i = Y_LEN-2; i >= 0 ; --i)
    {

        if (j > '9')
        {
            j='0';
            ++j2;
        }

        plain[i][1] = j++;
        if (j2 > '0')
            plain[i][0]=j2;
    }

    j = '0';
    for (int k = 10; k < X_LEN; k+=12)
    {

        if (j > '9')
        {
            plain[Y_LEN-1][k] = '1';
            plain[Y_LEN-1][k+1] ='0';
        }
        else
        {
            plain[Y_LEN-1][k] = j++;
        }
    }
}

void plot_graph(char plain[][X_LEN], int Y_LEN, int hist[])
{


   char plotting_styles[12]="O-*+#+@-*#+";

    for (int y = Y_LEN - 2; y >= 0; --y)
    {
        for (int x = 10, l = 0; x < X_LEN; x += 12, ++l)
        {
            for (int k = 0; k < 4; ++k)
                if(hist[l] > 0)
                    plain[y][x + k] = plotting_styles[l];

            hist[l]--;
        }
    }
}

void print_graph_on_screen(char plain[][X_LEN], int Y_LEN)
{
   char palette[7][7]={"[0m","[0;31m","[0;32m","[0;33m","[0;34m","[0;35m","[0;36m"};
   int iter = 0;
   int iter2;


    for (int k = 0; k < Y_LEN; ++k)
    {

        printf("\033%s",palette[0]);
        for (int m = 0; m < X_LEN; ++m)
        {
            if (k==Y_LEN-1);

            else
            {
                if (m == 2 + iter * 12)
                {
                    if (iter > 5)
                        iter2 = iter + 1 - 6;
                    else
                        iter2 = iter + 1;

                    printf("\033%s", palette[iter2]);
                    iter++;
                }
            }

            printf("%c", plain[k][m]);
        }
        printf("\n");
        iter = 0;
    }
}


void append_hist_to_file(char *f_name, int *hist, int len)
{
    FILE *fp = fopen(f_name,"a");


    if (fp == NULL)
     perror("Error: ");
    else
    {
        for (int i = 0; i < len; ++i)
        {
            fprintf(fp,"%d ",hist[i]);
        }
        fprintf(fp,"\n");
        fclose(fp);
    }
}

int get_rand()
{
     return rand() % 11;
}

void fill_arr_with_rand(int *arr, int len)
{
    srand((unsigned) time(&t));
    for (int i = 0; i < len; ++i)
    {
       arr[i] = get_rand();
    }
}